...
PARKS = {
    "ARI": (33.445, -112.067, 0,   True),  "AZ": (33.445, -112.067, 0, True),
    "ATL": (33.891, -84.468, 40,   False), "BAL": (39.284, -76.622, 30, False),
    "BOS": (42.346, -71.097, 55,   False), "CHC": (41.948, -87.656, 40, False),
    "CWS": (41.830, -87.634, 125,  False), "CIN": (39.097, -84.507, 120, False),
    "CLE": (41.496, -81.685, 0,    False), "COL": (39.756, -104.994, 5, False),
    "DET": (42.339, -83.049, 150,  False), "HOU": (29.757, -95.356, 345, True),
    "KC":  (39.051, -94.480, 45,   False), "LAA": (33.800, -117.883, 65, False),
    "LAD": (34.074, -118.240, 25,  False), "MIA": (25.778, -80.220, 40, True),
    "MIL": (43.028, -87.971, 130,  True),  "MIN": (44.982, -93.278, 90, False),
    "NYM": (40.757, -73.846, 15,   False), "NYY": (40.829, -73.926, 55, False),
    "ATH": (38.580, -121.513, 60,  False), "PHI": (39.906, -75.166, 10, False),
    "PIT": (40.447, -80.006, 115,  False), "SD": (32.707, -117.157, 0, False),
    "SEA": (47.591, -122.332, 45,  True),  "SF": (37.778, -122.389, 90, False),
    "STL": (38.623, -90.193, 60,   False), "TB": (27.768, -82.653, 45, True),
    "TEX": (32.747, -97.084, 135,  True),  "TOR": (43.641, -79.389, 345, True),
...
def weather_mults(park: str, date: str, hour_utc: int = 23) -> dict:
    """HR/hit multipliers from the game-time forecast. Neutral for roofed parks."""
    ...
    try:
        r = S.get("https://api.open-meteo.com/v1/forecast", timeout=15, params={
            "latitude": lat, "longitude": lon,
            "hourly": "temperature_2m,wind_speed_10m,wind_direction_10m",
            "temperature_unit": "fahrenheit", "wind_speed_unit": "mph",
            "start_date": date, "end_date": date}).json()
        hh = r["hourly"]
    ...
    return {"hr": round(hr, 3), "hit": round(hit, 3),
            "detail": f"{temp:.0f}F wind {wspd:.0f}mph out-comp {out_comp:+.0f}"}
...
def ump_k_mult(game_pk: int) -> float:
    ...
    try:
        ...
        return 1.0
    try:
        r = S.get(f"{config.STATSAPI_BASE}.1/game/{game_pk}/feed/live",
                  timeout=15).json()
...
def starter_pitch_limit(pid: int, season: int | None = None) -> int | None:
    ...
    try:
        r = S.get(f"{config.STATSAPI_BASE}/people/{pid}/stats", timeout=15,
...
def unavailable_relievers(team_id: int, date: str) -> dict:
    ...
    for back, w in ((1, 0.25), (2, 0.60)):
        ...
        try:
            sched = S.get(f"{config.STATSAPI_BASE}/schedule", timeout=15, params={
                "sportId": 1, "date": day, "teamId": team_id}).json()
            for dd in sched.get("dates", []):
                for g in dd.get("games", []):
                    box = S.get(f"{config.STATSAPI_BASE}/game/{g['gamePk']}/boxscore",
                                timeout=15).json()
